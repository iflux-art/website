...
export interface LinksStorage {
  ...
  addItem(
    item: Omit<LinksItem, "id" | "createdAt" | "updatedAt">,
  ): Promise<LinksItem>;
...
export class FileLinksStorage implements LinksStorage {
  async read(): Promise<LinksData> {
    const { readLinksData } = await import(
      "@/components/layout/links/admin/links-manage"
    );
  ...
  async write(data: LinksData): Promise<void> {
    const { writeLinksData } = await import(
      "@/components/layout/links/admin/links-manage"
    );
    const validCategoryIds = [
      "ai",
  ...
  async addItem(
    ...
    const { addLinksItem } = await import(
      "@/components/layout/links/admin/links-manage"
    );
    return addLinksItem({
      ...item,
  ...
  async updateItem(
    ...
    const { updateLinksItem } = await import(
      "@/components/layout/links/admin/links-manage"
    );
    const validCategoryIds = [
      "ai",
  ...
  async deleteItem(id: string): Promise<void> {
    const { deleteLinksItem } = await import(
      "@/components/layout/links/admin/links-manage"
    );
    deleteLinksItem(id);
  }
...
export class LinksService {
  ...
  // 验证链接数据
  validateLinkData(item: Partial<LinksItem>): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
