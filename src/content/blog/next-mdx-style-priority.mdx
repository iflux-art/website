...
export const mdxComponents = {
  ...
  h1: ({ children, id, ...props }) => {
    ...
    return <h1 id={headingId} className={cn(headingStyles.h1, props.className)} {...props}>{children}</h1>;
  },
  h2: ({ children, id, ...props }) => {
    const headingId = id || (typeof children === 'string' ? children.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '') : undefined);
    return <h2 id={headingId} className={cn(headingStyles.h2, props.className)} {...props}>{children}</h2>;
  },
