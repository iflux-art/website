...
<!-- 缩放淡出动画 -->
<span
  class="absolute h-3 w-3 animate-ping rounded-full bg-[oklch(0.6_0.2_20)] opacity-75"
></span>
...
<!-- 弹跳动画 -->
<div class="h-10 w-10 animate-bounce rounded-full bg-primary"></div>
```
...
```html
<div class="iteration-count-1 animate-bounce"><!-- 1次 --></div>
<div class="iteration-count-infinite animate-bounce"><!-- 无限次 --></div>
```
...
<div class="group hover:bg-blue-500">
  <div class="opacity-0 group-hover:animate-bounce group-hover:opacity-100">
    <!-- 当父元素悬停时显示并播放动画 -->
  </div>
</div>
```
...
module.exports = {
  theme: {
    extend: {
      ...
      animation: {
        "fade-in-delay":
          "fade-in-delay var(--duration, 0.5s) var(--delay, 0s) ease-out forwards",
      },
    },
  },
};
...
<div
  ...
  style="--offset: 20px; --duration: 0.8s; --delay: 0.2s;"
>
  自定义动画参数
</div>
```
...
```html
<button
  class="transform rounded bg-blue-500 px-4 py-2 font-bold text-white transition duration-300 hover:bg-blue-700 active:scale-95"
>
  点击我
</button>
```
...
```html
<div
  class="transform rounded-lg bg-white p-6 transition duration-300 ease-in-out hover:-translate-y-1 hover:shadow-lg"
>
