...
<div className={getCategoryColorClasses("blue")}>蓝色分类</div>;
```
...
<span className={getTagColorClasses("green")}>绿色标签</span>;
```
...
export function CustomColorCard({ title, description, hue = 240 }) {
  return (
    <div
      className={`rounded-xl border p-6 shadow-sm bg-[oklch(0.98_0.02_${hue}/0.2)] border-[oklch(0.85_0.05_${hue}/0.3)]`}
    >
      ...
      <button
        className={`mt-4 rounded-lg px-4 py-2 bg-[oklch(0.5_0.15_${hue})] text-[oklch(0.98_0_0)] hover:bg-[oklch(0.45_0.15_${hue})] transition-colors`}
      >
        了解更多
      </button>
    </div>
...
- [Tailwind CSS 完整指南](./tailwind-guide.mdx)
- [CSS 动画完全指南](./css-animations.mdx)
