...
import { ArrowLeft, ChefHat, Calculator, ShoppingCart, Timer } from 'lucide-react';
import Link from 'next/link';

export default function CookingToolkitPage() {
  const [activeTab, setActiveTab] = useState<'recipes' | 'nutrition' | 'shopping' | 'timer'>('recipes');
  ...
  const RecipeSearch = () => {
    ...
    const recipes = [
      { name: '宫保鸡丁', cuisine: '川菜', difficulty: '中等', time: '30分钟', ingredients: ['鸡肉', '花生', '辣椒'] },
      { name: '红烧肉', cuisine: '鲁菜', difficulty: '简单', time: '60分钟', ingredients: ['五花肉', '生抽', '老抽'] },
      { name: '麻婆豆腐', cuisine: '川菜', difficulty: '简单', time: '20分钟', ingredients: ['豆腐', '肉末', '豆瓣酱'] },
      { name: '糖醋里脊', cuisine: '鲁菜', difficulty: '中等', time: '25分钟', ingredients: ['里脊肉', '醋', '糖'] },
      { name: '蒸蛋羹', cuisine: '家常菜', difficulty: '简单', time: '15分钟', ingredients: ['鸡蛋', '温水', '盐'] }
    ];

    const filteredRecipes = recipes.filter(recipe => {
      const ingredientMatch = !ingredient || recipe.ingredients.some(ing => ing.includes(ingredient));
    ...
    return (
      <Card>
        <CardHeader><CardTitle>菜谱搜索</CardTitle></CardHeader>
  ...
  const NutritionCalculator = () => {
    ...
    return (
      <Card>
        <CardHeader><CardTitle>营养计算</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-3">
          ...
          <div className="grid grid-cols-4 gap-4">
            <div className="p-3 bg-red-50 rounded-lg text-center">
              <div className="text-lg font-bold text-red-600">{totalNutrition.calories.toFixed(1)}</div>
            ...
            <div className="p-3 bg-green-50 rounded-lg text-center">
              <div className="text-lg font-bold text-green-600">{totalNutrition.carbs.toFixed(1)}g</div>
              <div className="text-sm text-muted-foreground">碳水化合物</div>
            </div>
            <div className="p-3 bg-yellow-50 rounded-lg text-center">
              <div className="text-lg font-bold text-yellow-600">{totalNutrition.fat.toFixed(1)}g</div>
                ...
                <div key={category}>
                  ...
                  <div className="space-y-2">
                    {categoryItems.map((item, index) => {
                      ...
                      return (
                        <div key={globalIndex} className="flex items-center gap-3 p-2 border rounded-lg">
                          <input
                            type="checkbox"
  ...
  const CookingTimer = () => {
    ...
    return (
      <Card>
        <CardHeader><CardTitle>定时提醒</CardTitle></CardHeader>
        <CardContent className="space-y-4">
            ...
            {timers.map(timer => (
              <div key={timer.id} className="p-4 border rounded-lg">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className={`text-2xl font-mono ${timer.remaining === 0 ? 'text-red-600' : 'text-blue-600'}`}>
                    {formatTime(timer.remaining)}
                  </div>
                </div>
                <div className="flex gap-2">
                  {!timer.isRunning ? (
                    <Button size="sm" onClick={() => startTimer(timer.id)}>开始</Button>
                  ) : (
                    <Button size="sm" variant="outline" onClick={() => pauseTimer(timer.id)}>暂停</Button>
                  )}
                  <Button size="sm" variant="outline" onClick={() => resetTimer(timer.id)}>重置</Button>
                  <Button size="sm" variant="outline" onClick={() => removeTimer(timer.id)}>删除</Button>
                </div>
                {timer.remaining === 0 && (
