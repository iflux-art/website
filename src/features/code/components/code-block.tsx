...
export function CodeBlock({
  ...
  // 为了确保服务器端和客户端渲染一致，在服务器端预先添加tabindex属性
  // 这样在服务器端和客户端初始渲染时都会有这个属性，避免hydration错误
  // 然后在客户端通过useEffect移除它
  return (
    <div
      className={cn(
    ...
    >
      ...
      <div className="relative">
        {/* 自定义高亮行 */}
        {mounted && renderHighlightedLines()}
        ...
        >
          <code
            ref={codeRef}
            className={`${!mounted ? "text-foreground" : `language-${getLanguage()}`}`}
          >
            {code}
          </code>
        </pre>
