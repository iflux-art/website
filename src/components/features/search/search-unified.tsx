...
import { Search, X, FileText, Book, Home, Link as LinkIcon, Command } from "lucide-react";
 ...
 * 提供统一的搜索按钮样式和动画效果
 */
function BaseSearchButton({ onClick }: SearchButtonProps) {
  return (
    <Button
      variant="ghost"
      size="icon"
...
export function SearchButton({ onClick }: SearchButtonProps) {
  return (
    <Button
      variant="outline"
      size="sm"
      className="w-full justify-between text-sm text-muted-foreground sm:pr-12 md:w-40 lg:w-64"
      onClick={onClick}
    >
      <span>搜索...</span>
...
export function SearchCommandDialog() {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <SearchDialogContent className="p-0 max-w-[90vw] sm:max-w-[600px] overflow-hidden">
  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={setOpen}>
      <DialogPrimitive.Trigger asChild>
        <BaseSearchButton onClick={() => setOpen(true)} />
