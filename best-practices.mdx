...
“配置内容”指的是：实际参与运行时行为的常量、对象、枚举、配置表等（如导航菜单、缓存策略、元数据、路由映射等）。
常量（如路由、缓存时间、主题、导航、元数据等）
配置对象（如 API 路径、缓存策略、全局参数、组件映射等）
与业务无关、可全局复用、无副作用的静态数据
“类型定义”仅用于 TypeScript 静态类型检查，不属于配置。
